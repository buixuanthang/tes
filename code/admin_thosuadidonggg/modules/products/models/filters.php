		...
		if ($total) {
			...
			for($i = 0; $i < $total; $i ++) {
				...
				if ($filter_show && $alias) {
					$arr_insert [] = "('$filter_show','$alias','$table_name','$field_name','$field_show','$calculator','$value','$seo_title','$seo_meta_key','$seo_meta_des',$field_ordering,$ordering,$published,$is_common,$is_home)";
				}
			}
			// insert into table fs_products_tables
			if (count ( $arr_insert )) {
				$sql_insert .= " INSERT INTO " . $this->table_name . " ";
				$sql_insert .= "		(filter_show,alias,tablename,field_name,field_show,calculator,filter_value,seo_title,seo_meta_key,seo_meta_des,field_ordering,ordering,published,is_common,is_home) ";
				$sql_insert .= "		 VALUES ";
...
//			$field_ordering_exist_begin = FSInput::get ( 'field_ordering_exist_' . $i . '_begin', $field_current->ordering, 'int' );
			...
			$is_home_exist = FSInput::get ( 'is_home_exist_' . $i );
			$is_home_exist_begin = FSInput::get ( 'is_home_exist_' . $i . '_begin' );
			$is_home_exist = $is_home_exist ? 1 : 0;
			$is_home_exist_begin = $is_home_exist_begin ? 1 : 0;
