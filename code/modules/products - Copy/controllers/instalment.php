...
class ProductsControllersInstalment extends FSControllers {
		...
		if($id){
			...
			if(count($list)){
				foreach ($list as $key=>$item) {
					...
					$arr_products[$i]['price']=format_money($price ,' VNĐ');
					$arr_products[$i]['d_price']=$price;
					$arr_products[$i]['discount']=($discount)?format_money($discount,' VNĐ'):'0 VNĐ';
	...
	//			echo '</pre>';
		...
		global $tmpl;
		$title = 'Mua '.$data -> name.' lãi suất thấp';
	
		$tmpl->addTitle( $title);
		
		$tmpl->addMetakey($title);
		$tmpl->addMetades($title);


				$breadcrumbs = array ();

	
		$breadcrumbs[] = array(0=>'Trả góp', 1 => '');	
	...
	function  load_data(){
		...
		$html .='<span>';
		$html .=format_money($price ,' VNĐ');
		$html .='</span>';
		$html .='</div>';
		$html .='<div class="discount">';
		$html .='Khuyến mại: <span>'.format_money($discount ,' VNĐ').'</span>';
		$html .='</div>';
		$html .='</div>';
		echo $html;
		return;
	...
	function calculator(){
		...
		$html .='</div>';
		$html .='<div class="td-right">';
		$html .=format_money($price_dicker_before,' VNĐ');
		...
		$html .='</div>';
		$html .='<div class="td-right">';
		$html .=format_money($price_dicker_monthly,' VNĐ');
		...
		$html .=format_money($instalment_total,' VNĐ');
	...
	function alepay_result(){
		...
		// if($order_id) {
		// 	$link = FSRoute::_('index.php?module=products&view=cart&task=finished&id='.$order_id);
