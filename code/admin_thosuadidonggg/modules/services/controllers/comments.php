<?php
	class ServicesControllersComments extends Controllers{
...
//			$categories = $model->get_categories_tree_all();
		...
		function ajax_get_comments_by_service()
		{
			...
			$model = $this -> model;
			$data = $model->get_comments_by_service($record_id);
...
//			$data = $model->get_service_by_id($id);
//			if(!$data){
...
//			$this -> arr_status = array(1=>'Lưu nháp',2=>'BTV từ chối',3=>'Chờ BTV duyệt',4=>'Đã biên tập',5=>'Hạ bài (kéo về)',6=>'Xuất bản');
		...
		function view_comment($service){
			$link = 'index.php?module=services&view=comments&keysearch=&text_count=1&text0='.$news ->id.'&filter_count=1&filter0=0';
			$html = '';
			if($service -> comments_unread){
				$html .= '<strong>'.$service -> comments_unread.'/'.$service -> comments_total.'</strong>';
			}else{
				$html .= $service -> comments_unread.'/'.$service -> comments_total;
			}
