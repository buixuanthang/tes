		...
		function save_foreign($field_current,$filter_type = 'foreign_one'){
			...
			foreach($foreign as $item){
				if(in_array($item -> id, $arr_foreign_id_select)){
					...
					$check_exist = $this  -> get_count(' is_common = 1  AND field_name = "'.$field_current -> field_name .'" AND filter_value = "'.$item -> id.'"',$this -> table_name);
					if(!$check_exist){
						$row = array();
			...
			$sql = " DELETE FROM ".$this -> table_name." 
						WHERE  is_common = 1 AND field_name = '".$field_current -> field_name ."' ".$sql_where ;
			global $db;
			$db->query($sql);
			$rows = $db->affected_rows();
			if(!$rows)
