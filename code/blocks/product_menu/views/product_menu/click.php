...
foreach ( $list as $item ) {
	...
	if ($item->level) {
		...
		if ($group_activated) {
			...
			$html_normal .= "<h3 class='h3_" . $item->level . " active'><a href='" . $link . "' ><span> " . $item->name . "</span></a></h3>  ";
		} else {
			$html_normal .= "<li class='item $class  ' id='pr_" . $item->id . "' >";
			$html_normal .= "<h3 class='h3_" . $item->level . "'><a href='" . $link . "' ><span> " . $item->name . "</span></a></h3>  ";
		}
	}
