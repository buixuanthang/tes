...
class ConfigModelsModules extends FSModels {
	...
	function __construct() {
		parent::__construct ();
		$this->table_name = FSTable_ad::_('fs_config_modules');
	}
	
	function getData() {
		return $this->get_records ( 'published = 1', $this->table_name, '*', 'module  ASC, ordering ASC' );
	}
	/*
		 * 
		 * Save
...
//			include_once '../blocks/' . $module . '/config.php';
		...
		foreach ( $params as $key => $value ) {
			...
			if ($value ['type'] == 'text') {
				...
				$str_params .= $key . '=' .json_encode(FSInput::get ( 'params_' . $key ));
			}
			$i ++;
		}
		return $str_params;
