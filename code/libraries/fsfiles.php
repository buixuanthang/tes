		...
		if(file_exists($filelogo)){
			...
			if($insertLogo==1){
				...
				switch($vitri){
					...
					case 2:
						$dpy 	= 5;
						$dpx 	= $width_image/2-($width_logo_new/2);
					break;
					//Bên phải trên ảnh
					case 3:
						$dpy 	= 5;
						$dpx 	= $width_image-($width_logo_new+5);
					break;
					//Bên trái giữa ảnh
					case 4:
						$dpy 	= $height_image/2-($height_logo_old/2+5);
						$dpx 	= 5;
					break;
					//Bên phải giữa ảnh
					case 6:
					...
					//Bên trái dưới ảnh
					case 7:
						$dpy 	= $height_image-($height_logo_old);
						$dpx 	= 0;
					break;
					//Giữa dưới ảnh
					case 8:
						$dpy 	= $height_image-($height_logo_old+5);
						$dpx 	= $width_image/2-($width_logo_new/2);
					break;
					//Bên phải dưới ảnh
					case 9:
						$dpy 	= $height_image-($height_logo_new);
						$dpx 	= $width_image-($width_logo_new);
					break;
					//CĂN GIỮA
					default:
						$dpy 	= $height_image/2-($height_logo_old/2);
						$dpx 	= $width_image/2-($width_logo_new/2);
					break;
				}
				$sx 	= $width_logo_old;
				...
				// mặc định logo luôn là .png
				$buffer_logo = imagecreatefrompng($filelogo);	
				
				$out = imagecreatetruecolor($width_image, $height_image);
 				imagecopyresampled ($out, $image      , 0, 0, 0, 0, $width_image, $height_image, $width_image, $height_image);
 				imagecopyresampled ($out, $buffer_logo,$dpx, $dpy, 0, 0, $width_logo_new  , $height_logo_new, $width_logo_old, $height_logo_old);

				switch ($sExtension){
				case "jpg" :
					imagejpeg($out, $path . $filename,100);
					break;
				case "gif" :
					imagegif($out, $path . $filename,100);
					break;
				case "png" :
					imagesavealpha( $out, true );
					imagefill( $out, 0, 0, imagecolorallocatealpha( $out, 0, 0, 0, 127 ) );
					
					imagepng($out, $path. $filename,9);
					break;
				}
				imagedestroy($out);
			}
