							...
							<table border="1" class="tbl_form_contents">
								<thead>
									...
									<th class="title" width="27%">
										<?php echo  TemplateHelper::orderTable('Tiêu đề', 'a.title',$sort_field,$sort_direct) ; ?>
									</th>
								...
								<tbody>
									...
									<?php if($count){?>
										<?php foreach ($list as $row) { ?>
											<?php $link_detail = FSRoute::_('index.php?module=services&view=service&id='.$row->id.'&code='.$row -> alias.'&ccode='.$row-> category_alias); ?>
											<tr class="row<?php echo $i%2; ?> service_record" id="record_<?php echo $row -> id; ?>">
												<td align="left">
					...
					<td width="70%">
						<table border="1" class="tbl_form_contents">
							<thead>
								<th class="title" >
									<td width="70%">
										<div class='service_title' id='service_title'></div>
									</td>
									<td width="30%">
...
//			$.get("index2.php?module=services&view=comments&task=ajax_get_comments_by_service&raw=1",{record_id:record_id}, function(html){
//				$('#comments_area').html(html);
//				$('#unread_'+record_id).html(0);
//				redisplay_comments_static_on_tabhead(record_id);
//			});
			$('.service_record').removeClass('activated');
			$(this).addClass('activated');
		});
	}
	function ajax_call_comments(record_id){
		type = $('#type_comment').val();
		$.get("index2.php?module=services&view=comments&task=ajax_get_comments_by_service&raw=1",{record_id:record_id,type:type}, function(html){
			$('#comments_area').html(html);
			$('#unread_'+record_id).html(0);
			redisplay_comments_static_on_tabhead(record_id);
		});
	...
	function ajax_unpublished(id,record_id){
		$.get("index2.php?module=services&view=comments&task=ajax_unpublished&raw=1",{id:id,record_id:record_id}, function(status){
			if(status == 1){
				type = $('#type_comment').val();
				$.get("index2.php?module=services&view=comments&task=ajax_get_comments_by_service&raw=1",{record_id:record_id,type:type}, function(html){
//				$.get("index2.php?module=services&view=comments&task=ajax_get_comments_by_service&raw=1",{record_id:record_id}, function(html){
	...
	*/
	...
	function ajax_published(id,record_id){
		$.get("index2.php?module=services&view=comments&task=ajax_published&raw=1",{id:id,record_id:record_id}, function(status){
			if(status == 1){
				type = $('#type_comment').val();
				$.get("index2.php?module=services&view=comments&task=ajax_get_comments_by_service&raw=1",{record_id:record_id,type:type}, function(html){
//				$.get("index2.php?module=services&view=comments&task=ajax_get_comments_by_service&raw=1",{record_id:record_id}, function(html){
	...
	function ajax_del(id,record_id,published){
		if(confirm('Bạn chắc chắn muốn xóa?')){
			$.get("index2.php?module=services&view=comments&task=ajax_del&raw=1",{id:id,record_id:record_id}, function(status){
				if(status == 1){
					type = $('#type_comment').val();
					$.get("index2.php?module=services&view=comments&task=ajax_get_comments_by_service&raw=1",{record_id:record_id,type:type}, function(html){
//					$.get("index2.php?module=services&view=comments&task=ajax_get_comments_by_service&raw=1",{record_id:record_id}, function(html){
	...
	function submit_reply(parent_id,record_id){
		...
		$.get("index.php?module=services&view=comments&task=ajax_save_comment&raw=1",{record_id:record_id,parent_id:parent_id,name: name,text:reply_content}, function(status){
			if(status == 1){
				type = $('#type_comment').val();
				$.get("index2.php?module=services&view=comments&task=ajax_get_comments_by_service&raw=1",{record_id:record_id,type:type}, function(html){
//				$.get("index2.php?module=service&view=comments&task=ajax_get_comments_by_service&raw=1",{record_id:record_id}, function(html){
					$('#comments_area').html(html);
				});
				no_published = $('#published_'+record_id).html();
				no_published ++ ;
	...
	function submit_edit(comment_id,record_id){
		...
		$.get("index.php?module=services&view=comments&task=ajax_edit_comment&raw=1",{record_id:record_id,comment_id:comment_id,name: name,text:edit_content}, function(status){
			if(status == 1){
				type = $('#type_comment').val();
				$.get("index2.php?module=services&view=comments&task=ajax_get_comments_by_service&raw=1",{record_id:record_id,type:type}, function(html){
//				$.get("index2.php?module=service&view=comments&task=ajax_get_comments_by_service&raw=1",{record_id:record_id}, function(html){
...
.service_list .activated a,.service_list .activated span,.service_list .activated{
	color: red;
	font-weight: bold;
}
#service_title {
