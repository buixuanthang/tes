...
//						$code2 =  isset($array_params['code2'])?$array_params['code2']: '';
					...
					case 'promotion':
						...
						return $url_first.'khuyen-mai.html'.$url1;	
					case 'hotdeal':
						foreach($array_params as $key=>$value){
							if($key == 'module' || $key == 'view' || $key == 'Itemid')
								continue;
						...
						return $url_first.'hotdeal.html'.$url1;		
					case 'countdown':
						foreach($array_params as $key=>$value){
							if($key == 'module' || $key == 'view' || $key == 'Itemid')
								continue;
						...
						return $url_first.'dau-gia-nguoc.html'.$url1;	
					case 'bought':
						foreach($array_params as $key=>$value){
							if($key == 'module' || $key == 'view' || $key == 'Itemid')
								continue;
						...
						return $url_first.'danh-sach-da-mua.html'.$url1;		
					case 'cart':
						switch ($task){
							case 'eshopcart2':
					...
					case 'favourites':
						return $url_first.'san-pham-yeu-thich.html';
					default:
						return $url_first.$url;
				}
				break;
			case 'news':
				switch ($view){
					...
					case 'cat':
						...
						$amp =  isset($array_params['amp'])?$array_params['amp']: '';
						$cid =  isset($array_params['cid'])?$array_params['cid']: '';
						return $url_first.$ccode.'-cn'.$cid.$suf;
					case 'home':
			...
			case 'contents':
				...
				$ccode =  isset($array_params['ccode'])?$array_params['ccode']: '';
					$cid =  isset($array_params['cid'])?$array_params['cid']: '';
					$suf = $amp?'.amp':'.html';
					return $url_first.$ccode.'-cct'.$suf;	
				case 'content':
				$code =  isset($array_params['code'])?$array_params['code']: '';
					$id =  isset($array_params['id'])?$array_params['id']: '';
					$ccode =  isset($array_params['ccode'])?$array_params['ccode']: '';
					return $url_first.$code.'-ct'.$id.$suf;
				}
				break;						
	case 'projects':
				switch ($view){
					case 'home':
						...
						if(!$type)
							return $url_first.'du-an'.'.html';
						else{
							return $url_first.'du-an'.'/'.$type.'.html';	
						}
					case 'project':
						$code =  isset($array_params['code'])?$array_params['code']: '';
						$id =  isset($array_params['id'])?$array_params['id']: '';
						$ccode =  isset($array_params['ccode'])?$array_params['ccode']: '';
						return $url_first.$ccode.'/'.$code.'-dpj'.$id.'.html';
					case 'search':
						$keyword =  isset($array_params['keyword'])?$array_params['keyword']: '';
					...
					case 'cat':
						...
						$trans_type =  isset($array_params['trans_type'])?$array_params['trans_type']: 1;
						if(!$type)
							return $url_first.$ccode.'-pj.html';
						else{
							return $url_first.$ccode.'-pj/'.$type.'.html'; 
						}
		...
		case 'documents':
				switch ($view){
					case 'home':				
						return $url_first.'tai-lieu'.'.html';						
					case 'document':
						if($task == 'download'){
							$data  = isset($array_params['data'])?$array_params['data']: '';
							return URL_ROOT.'download-doc/'.$data.'.html';
						}else{
							$code =  isset($array_params['code'])?$array_params['code']: '';
							$id =  isset($array_params['id'])?$array_params['id']: '';
							$ccode =  isset($array_params['ccode'])?$array_params['ccode']: '';
							return $url_first.$ccode.'/'.$code.'-dcm'.$id.'.html';		
						}
					...
					case 'cat':
						$ccode =  isset($array_params['ccode'])?$array_params['ccode']: '';
						return $url_first.$ccode.'-dcc.html';
	...
	case 'videos' :
			...
			case 'download_file':
				...
				case 'download_file':
					if($task == 'download'){
						$data  = isset($array_params['data'])?$array_params['data']: '';
							return URL_ROOT.'download/'.$data.'.html';
					} else{
						return $url_first.'download-file.html';
					}
				}
				break;	
			case 'advices':
		...
		case 'contact':
			...
			case 'calculation_power':
				return $url_first.'tinh-cong-suat-dieu-hoa.html';
				...
				switch ($view){
					case 'users':
						switch ($task){
							...
							case 'edit':
								return URL_ROOT.'sua-thong-tin-ca-nhan.html';
							case 'user_info':
