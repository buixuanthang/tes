<?php 
	class ServicesModelsComments extends FSModels{
...
//		}
	...
	function get_comments_by_service($record_id) {
		global $db;
		if (! $record_id)
			return;
	...
	function ajax_published($published = 1) {
		...
		$rs = $this->_update ( $row, 'fs_services_comments', 'id = ' . $id . ' AND record_id =  ' . $record_id . ' ' );
		echo $rs ? 1 : 0;
		if ($rs) {
	...
	function ajax_del() {
		...
		$rs = $this->_remove ( 'id = ' . $id . ' AND record_id =  ' . $record_id . ' ', 'fs_services_comments' );
		echo $rs ? 1 : 0;
		if ($rs) {
			$this->recal_comments ( $record_id );
		}
	...
	function recal_comments($record_id) {
		$list = $this->get_records ( 'record_id = ' . $record_id, 'fs_services_comments', 'id,published' );
		$total_published = 0;
		...
		$rs = $this->_update ( $row, 'fs_services', 'id = ' . $record_id . ' ' );
		return $rs;
	}
	function update_unread_for_comments($record_id) {
		$row ['readed'] = 1;
		$this->_update ( $row, 'fs_services_comments', 'record_id = ' . $record_id );
		
		$row2 ['comments_unread'] = 0;
		$this->_update ( $row2, 'fs_services', 'id = ' . $record_id . ' ' );
		return;
	}
	...
	function save_comment() {
		...
		$rs = $this -> _add($row, 'fs_services_comments');
		echo $rs?1:0;
		if ($rs)
			$this->recal_comments ( $record_id );
		return $rs;
...
//		$row ['created_time'] = $time;
		...
		$rs = $this -> _update($row, 'fs_services_comments',' id = '.$comment_id);
		echo $rs?1:0;
		if ($rs)
			$this->recal_comments ( $record_id );
		return $rs;
