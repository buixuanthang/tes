...
class TemplateHelper
{
    ...
    public static function  jscolorpicker($title, $name, $value, $default = 'FFFFFF', $size = 10, $comment = '',$class_col1='col-md-2',$class_col2='col-md-10') {
		if (! isset ( $value )) $value = $default;
		//$value = escape($value);
        echo '<script type="text/javascript" src="templates/default/js/jscolor.js"></script>';
		echo '<div class="form-group">';
		echo '	<label class="'.$class_col1.' col-xs-12 control-label" >' . $title . '</label>';
		echo '	<div class="'.$class_col2.' col-xs-12">';
	 ...
	 * 
	 */ 
	public function genarate_form_liting($module,$view,$list,$fitler_config,$list_config,$sort_field,$sort_direct,$pagination = null,$params = array(),$first='index.php'){
					...
					foreach($col as $item){
						...
						switch($type){
							...
							case 'text_in_array':
								...
								$field_value = isset($item['arr_params']['field_value'])?$item['arr_params']['field_value']: 'id';
								$field_label = isset($item['arr_params']['field_label'])?$item['arr_params']['field_label']: 'name';
								$html_body .= TemplateHelper::text_in_array($item['field'],$row->$item['field'],$i,$arry_select,$field_value,$field_label);
								break;
							case 'image':
                            ...
                            case 'text_link':
                                ...
                                if($link_web){
                                    ...
                                    $html_body .= '<a target="_blank" href="'.$link_web.'">'.$row -> $item['field'].'</a>';
                                }
                                break; 
...
//				$compare = $value;
		...
		if($sub_item)
			$html .= $sub_item;
		if($comment)
			$html .= '<span class="help-block">'.$comment.'</span>';
		$html .= '</div></div>';
		echo $html;
	}
	/*
...
//				echo "<script>CKEDITOR.replace( '".$name."');</script>";
			...
			if($name == 'price' || $name=='price_old'|| $name=='discount' ||  $name=='price_min'){
				if($value)
		...
		if($sub_item)
			echo  $sub_item;
		if($comment)
			echo '<span class="help-block">'.$comment.'</span>';
		echo  '</div></div>';
        //echo $html;
	}
	/*
	...
	function dt_edit_money($title,$name,$value,$default = '',$size = 60,$rows = 1,$editor = 0,$comment='',$sub_item = ''){
		if(!isset($value))
			$value = $default;
		echo '<tr><td valign="top" class="label key" valign="top">'.$title.'</td><td class="value">';
	   echo '<input type="text" name="'.$name.'" id="'.$name.'" value="'.format_money($value,' ', '0').'" size="'.$size.'"/>';
		if($sub_item)
			echo  $sub_item;
		if($comment)
	 ...
	 */
	function sub_edit_text($title,$name,$value,$default = '',$size = 60,$rows = 1,$class_col1='col-md-2',$class_col2='col-md-10'){
		if(!isset($value))
			$value = $default;
		$html = '<span>'.$title.'</span>';
		if($rows > 1){
	 ...
	 * FOR DETAIL PAGE
	 */
	function dt_text($title,$value,$default = '',$comment='',$use_htmlspecialchars = 1,$class_col1='col-md-2',$class_col2='col-md-10'){
		if(!isset($value))
			$value = $default;
		echo '<div class="form-group">
                    <label class="'.$class_col1.' col-xs-12 control-label">'.$title.'</label>
...
/*
	 ...
	 * in case editor: width => size, height => rows
	 */
	function dt_image($title,$name,$value,$width  = 0,$height = 0,$comment='',$class_col1='col-md-2',$class_col2='col-md-10'){
		$html = '<div class="form-group">
                    <label class="'.$class_col1.' col-xs-12 control-label">'.$title.'</label>
                    <div class="'.$class_col2.' col-xs-12">';
