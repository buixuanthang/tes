<?php 
	class ProjectsModelsProjects extends FSModels
	{
		var $limit;
		var $prefix ;
		function __construct()
		{
			...
			$this -> view = 'projects';
			$this -> type = 'projects';
			$this -> table_name = FSTable_ad::_ ('fs_projects');
			$this -> table_name_imgage = FSTable_ad::_ ('fs_projects_images');
			$this -> table_category = FSTable_ad::_ ('fs_'.$this -> type.'_categories');
			
			$this -> arr_img_paths = array(array('resized',285,285,'cut_image'),array('large',456,320,'cut_image'),array('small',90,80,'cut_image'));
			$this -> arr_img_paths_other = array(array('large',456,320,'cut_image'),array('small',90,80,'cut_image'));
			// config for save
			$cyear = date('Y');
			$cmonth = date('m');
			$cday = date('d');
...
//		}
		...
		function remove_other_images($record_id){
			...
			if($str_other_images)
			{
				...
				// remove in database
				$sql = " DELETE FROM ".$this -> table_name_imgage."
						WHERE record_id = $record_id
							AND id IN ($str_other_images)" ;
				$db->query($sql);
...
//			// remove main image
//			$img_paths = array();
//			$img_paths[] = PATH_IMG_PROJECTS_ORIGINAL;
//			$img_paths[] = PATH_IMG_PROJECTS_STANDART;
//			$img_paths[] = PATH_IMG_PROJECTS_CROPPED;
...
//		}
		...
		function _______remove_image_by_record_id()
		{
			...
			foreach ($images_need_remove as $item) {
				if($item->image)
				{
					$fsFile-> remove($item->image, PATH_IMG_PROJECTS_ORIGINAL);
					$fsFile-> remove($item->image, PATH_IMG_PROJECTS_STANDART);
					$fsFile-> remove($item->image, PATH_IMG_PROJECTS_CROPPED);
				}
			}
		...
		function save_exist_images(){
			...
			for($i= 0 ; $i < $images_exist_total ; $i++){
				...
				if( ($name_exist != $name_exist_begin) || ($ordering_exist != $ordering_exist_begin) ) {
					...
					$u = $this -> _update($row, $this -> table_name_imgage,' id='.$id_exist);
					if($u)
						$rs ++;
				}
			}
	...
	function update_other_images($id = 0) {
		...
		$query = '  UPDATE '.$this -> table_name_imgage.' SET record_id = ' . $id . ', session_id = \'\'
	...
	function upload_other_images() {
		...
		$rs = $this->_add ( $row, $this -> table_name_imgage );
		$row ['id'] = $rs;
	...
	function change_attr_image() {
		...
		$rs = $this->_update ( $row, $this -> table_name_imgage, ' id = ' . $id . $where );
		return $rs;
	}
	function delete_other_image($record_id = 0) {
		...
		if ($listImages) {
			foreach ( $listImages as $item ) {
				$query = '  DELETE FROM '.$this -> table_name_imgage.'
                                WHERE id = \'' . $item->id . '\'';
				$db->query ( $query );
		...
		if (isset ( $_POST ["sort"] )) {
			if (is_array ( $_POST ["sort"] )) {
				foreach ( $_POST ["sort"] as $key => $value ) {
					$db->query ( "UPDATE ".$this -> table_name_imgage." SET ordering = $key WHERE id = $value" );
				}
			}
		}
	}
