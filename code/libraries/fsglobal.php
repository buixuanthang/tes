	...
	function get_module_config($module,$view = '',$task = '',$params = array()){
		...
		if(USE_MEMCACHE){
			...
			if($config_in_memcache){
				...
				$fstable  = FSFactory:: getClass('fstable');
				global $db;
				$sql = " SELECT cache,params,fields_seo_title,fields_seo_keyword,fields_seo_description,fields_seo_h1,fields_seo_h2,fields_seo_image_alt,value_seo_title,value_seo_keyword,value_seo_description  FROM ".$fstable->_('fs_config_modules')."
					WHERE $where ";
				$rs =  $db->getObject($sql);
	...
	function get_blocks(){
		...
		if(USE_MEMCACHE){
			...
			if($blocks_in_memcache){
				...
				global $db;
				$sql = " SELECT id,title,content, ordering, module, position, showTitle, params ,listItemid,news_categories,contents_categories,link_title
