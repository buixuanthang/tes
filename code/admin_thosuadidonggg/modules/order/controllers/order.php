<?php
	...
	// Excel toàn bộ danh sách copper ra excel
		...
		function export_file(){
			...
			$list = $model->get_member_info(0,100);
//			print_r($list);die;
			if(empty($list)){
				echo 'error';exit;
			}else {
...
'bold' => true,
			...
			$excel->obj_php_excel->getActiveSheet()->getColumnDimension('A')->setWidth(20);
				...
				$excel->obj_php_excel->getActiveSheet()->setCellValue('D1', 'Giá trị');
				$excel->obj_php_excel->getActiveSheet()->setCellValue('E1', 'Ngày mua');
				foreach ($list as $item){
					...
					$excel->obj_php_excel->getActiveSheet()->setCellValue('D'.$key, format_money($item -> total_after_discount));
		...
		function desc_order_code($data){

			$estore_code = 'DH';
			$estore_code .= str_pad($data -> id, 8 , "0", STR_PAD_LEFT);
			$link_view = "index.php?module=".$this -> module."&view=".$this -> view."&task=edit&id=".$data->id;
