...
<?php 
	class ProductsModelsInstalment extends FSModels
	{
		...
		function  get_product(){
			...
			$select = " id,name,summary,image,price,price_old,quantity,alias,category_alias, discount,manufactory_alias,manufactory_image,manufactory_name,summary_auto ,types,promotion_info,published_double,warranty,published_double,date_start,date_end,is_hotdeal";
		...
		function alepay_eshopcart_save(){
			...
			$product_list[] = array($product_id,$price,$quantity,$color_id,$memory_id,$warranty_id,$origin_id,$species_id,$usage_states_id);
		...
		function save_order_items($order_id){
			...
			for($i = 0; $i < count($product_list); $i ++) {
			   ...
			   // calculator status
				...
				$array_insert[] = "('$order_id','$prd[0]','$prd[1]','$prd[2]','$total_money','$prd[3]','$color_name','$color_price','$prd[4]','$memory_name','$memory_price','$prd[5]','$warranty_name','$warranty_price','$prd[6]','$origin_name','$origin_price','$prd[7]','$species_name','$species_price','$prd[8]','$usage_states_name','$usage_states_price') ";
			}
