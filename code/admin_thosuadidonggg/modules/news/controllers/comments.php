<?php
	class NewsControllersComments extends Controllers{
		...
		function ajax_get_comments_by_news()
		{
			...
			$model = $this -> model;
			$data = $model->ajax_get_comments_by_news($record_id);
			...
			foreach ($data as $item){
				$html .= '<div class="comment-item comment-item-'.$item -> id.' '.($item -> parent_id? "comment-child":"") .'">';	
					$html .= '<div class="comment_info">';
						$html .= '<div class="comment_head"><span class="name" >'. $item -> title.'</span> ';	
					...
					$html .= '<div class="text_area_ct">';		
					$html .= '<input type="text" id="name_'.$item -> id.'" onfocus="if(this.value==\'Họ tên\') this.value=\'\'" onblur="if(this.value==\'\') this.value=\'Họ tên\'" value="Losiho" size="63" /><br/>';
...
//			$data = $model->get_project_by_id($id);
//			if(!$data){
